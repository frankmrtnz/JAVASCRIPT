...
function cargarTemporal(){
   $.get("./js/ofertas.json",{},(resultado)=> {
     listaOfertas = resultado;
        ordenarLista(resultado.temporal);
        pintarTemporal(resultado.temporal);
...
//Funcion para pintar las ofertas de trabajo Temporal mediante una lista que obtenemos del JSON 'ofertas.json'
   function pintarTemporal(lista){
    lista.forEach( resultado =>{
      ...
      document.getElementById("contenidoTemporal").innerHTML+= texto; 
      });
   }
...
//Funcion para ordenar los elementos de la lista de ofertas Temporal según el valor del select opt
   function ordenarLista(){
      document.getElementById("contenidoTemporal").innerHTML = "";
      var valueSelectOrdenar = document.getElementById("ordenarOfertas").value;
      var listaTrabajosTemporal=[];
      listaTrabajosTemporal.push(listaOfertas["temporal"]);
      

      if(valueSelectOrdenar == "nombre") {
        var ordenadaNombre = listaTrabajosTemporal[0].sort(function ordenar(a, b) { 
          return b.titulo < a.titulo ?  1 
              : b.titulo > a.titulo ? - 1 
              : 0;              
        });
        pintarTemporal(ordenadaNombre);
      }

      if(valueSelectOrdenar == "masAlto"){
        var ordenadaPrecioAlto = listaTrabajosTemporal[0].sort(function ordenar(a, b) { 
          return b.salario > a.salario ?  1 
              : b.salario < a.salario ? - 1 
              : 0;              
        });
        pintarTemporal(ordenadaPrecioAlto);
      }

      if(valueSelectOrdenar == "masBajo"){
        var ordenadaPrecioBajo = listaTrabajosTemporal[0].sort(function ordenar(a, b) { 
          return b.salario < a.salario ?  1 
              : b.salario > a.salario ? - 1 
              : 0;              
        });
        pintarTemporal(ordenadaPrecioBajo);
      }
   }
